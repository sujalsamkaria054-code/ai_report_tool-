...
def extract_tables(content: bytes) -> list[TableSpec]:
    ...
    """
    if not content:
        logger.info("Table extraction skipped: empty payload.")
        return []

    text = content.decode("latin-1", errors="ignore")
    if _looks_like_binary(text):
        logger.warning("Table extraction skipped: payload looks binary/unreadable.")
        logger.info("Table extraction stats | extracted=0 normalized=0 skipped_unreadable=1")
        return []
    ...
    extracted_count = 1 if len(candidate_rows) >= 2 else 0
    if extracted_count == 0:
        logger.info("Table extraction stats | extracted=0 normalized=0 skipped_unreadable=0")
        return []
    ...
    if normalized is None:
        ...
        logger.info("Table extraction stats | extracted=1 normalized=0 skipped_unreadable=1")
        return []

    logger.info("Table extraction stats | extracted=1 normalized=1 skipped_unreadable=0")
    return [normalized]
